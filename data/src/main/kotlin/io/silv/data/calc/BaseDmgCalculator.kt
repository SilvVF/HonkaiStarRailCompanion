...
class BaseDmgCalculator {
    ...
    private fun AttributeModifier.sumOf(attribute: Attribute) =
        filter { it.first == attribute }.sumOf { it.second }

    private operator fun AttributeModifier.get(attribute: Attribute): Double {
        return this.firstOrNull { attribute == it.first }?.second ?: 0.0
    }
    ...
    fun calculate(
        ...
        val baseStats = base.copy(
            hp = (base.hp + lc.baseHP) * (1 + pctStats[HP]) + flatStats.sumOf(HP),
            atk = (base.atk + lc.baseATK) * (1 + pctStats[ATK]) + flatStats.sumOf(ATK),
            def = (base.def  + lc.baseDEF) * (1 + pctStats[DEF]) + flatStats.sumOf(DEF),
